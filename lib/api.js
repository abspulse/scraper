...
export const init = (crawler, indexer) => {
  ...
  app.put('/api/crawler', (req, res) => {
    if (req.body.crawlInterval) {
      crawler.setCrawlInterval(parseInt(req.body.crawlInterval, 10))
    }
    if (req.body.saveInterval) {
      crawler.setSaveInterval(parseInt(req.body.saveInterval, 10))
    }
