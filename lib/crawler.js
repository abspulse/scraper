...
const getNewItemsToQueue = () => {
  ...
  setTimeout(() => getNewItemsToQueue(), 20 * 1000)
  ...
  CarModel.find(getQuery()).limit(2000).exec().then(cars => {
    if (cars.length) {
      queue = [
        ...queue,
        ...cars
  ...
  .then(() => {
    CarModel.find(getQuery()).count().exec().then(itemCount => {
      itemsLeftToCrawl = itemCount
    })
  })
}
...
export const getStats = () => {
  ...
  return CarModel.find({removed: null}).count().exec().then(unSoldCount =>
    ({
      itemsLeftToCrawl: itemsLeftToCrawl + queue.length,
